...
class LivreController {
    ...
    public function ajouterImage($image, $repertoire) {
        ...
        $extensionsTab = ['png', 'wedp', 'jpeg','jpg'];

        if (!in_array($extension, $extensionsTab))
            throw new Exception("Extension non autorisée => ['png', 'wedp', 'jpeg','jpg']"); // les formats autorisés
    ...
    public function validationModifierLivre(){
        ...
        $this->repositoryLivres->modifierLivreBdd($_POST['titre'], (int)$_POST['nbre-de-pages'], $_POST['text_alternatif'], $imageActuelle, $idLivre);
        header('location: ' . SITE_URL . 'livres');
    }
